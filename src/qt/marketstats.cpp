...
const QString kBaseUrl3 = "http://bittrex.com/api/v1/public/getorderbook?market=BTC-COIN2&type=both&depth=50";
const QString kBaseUrl4 = "https://poloniex.com/public?command=returnTicker";
const QString kBaseUrl5 = "http://www.poloniex.com/public?command=returnOrderBook&currencyPair=BTC_NC2";
const QString kBaseUrl6 = "http://bittrex.com/api/v1/public/getmarkethistory?market=BTC-COIN2&count=100";
const QString kBaseUrl7 = "https://poloniex.com/public?command=returnTradeHistory&currencyPair=BTC_NC2";
...
MarketStats::MarketStats(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::MarketStats)
{
    ui->setupUi(this);
    ...
    ui->customPlot2_2->addGraph();
    ui->customPlot2_2->setBackground(Qt::transparent);
...
void MarketStats::bittrex()
{
    QDesktopServices::openUrl(QUrl("https://www.bittrex.com/Market/Index?MarketName=BTC-COIN2"));
}

void MarketStats::poloniex()
{
    QDesktopServices::openUrl(QUrl("https://poloniex.com/exchange/btc_nc2"));
}

void MarketStats::mintpal()
{
    QDesktopServices::openUrl(QUrl("https://www.mintpal.com/market/C2/BTC"));
}

void MarketStats::bter()
{
    QDesktopServices::openUrl(QUrl("https://bter.com/trade/c2_btc"));
}

void MarketStats::randomChuckNorrisJoke()
{
    getRequest(kBaseUrl);
...
void MarketStats::getRequest( const QString &urlString )
{
    QUrl url ( urlString );
...
void MarketStats::parseNetworkResponse(QNetworkReply *finished )
{
...
if (what == kBaseUrl)
{
    ...
    QStringList data2 = data.split("{\"MarketName\":\"BTC-COIN2\",\"High\":");
...
if (what == kBaseUrl4)
{
    ...
    QStringList data2 = data.split("\"BTC_NC2\":{\"last\":\"");
    ...
    if(last[0] > lastp2)
    {
        ...
        ui->lastu_2->setText("<b><font color=\"green\">" + lastus + " $</font></b>");
    } else if (last[0] < lastp2) {
        ui->last_2->setText("<b><font color=\"red\">" + last[0] + "</font></b>");
         ui->lastu_2->setText("<b><font color=\"red\">" + lastus + " $</font></b>");
        } else {
    ...
    if(ask[0] > askp2)
    {
        ui->high_2->setText("<b><font color=\"green\">" + ask[0] + "</font></b>");
    } else if (ask[0] < askp2) {
        ui->high_2->setText("<b><font color=\"red\">" + ask[0] + "</font></b>");
        } else {
    ui->high_2->setText(ask[0]);
    }
    ...
    if(bid[0] > bidp2)
     {
         ui->low_2->setText("<b><font color=\"green\">" + bid[0] + "</font></b>");
     } else if (bid[0] < bidp2) {
         ui->low_2->setText("<b><font color=\"red\">" + ask[0] + "</font></b>");
         } else {
     ui->low_2->setText(bid[0]);
     }
    ...
    if(PrevDay[0].toDouble() > 0)
    {
        ...
        ui->yest_3->setText("<b><font color=\"green\"> + " + yestus + " %</font></b>");
    ...
    }else
    {
        yestu = PrevDay[0].toDouble()*100;
        yestus = QString::number(yestu);
        ui->yest_3->setText("<b><font color=\"red\">" + yestus + " %</font></b>");
...
void MarketStats::setModel(ClientModel *model)
{
    this->model = model;
}
